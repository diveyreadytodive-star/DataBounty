...
test('inactive Groq reference variables do not change the local MLX provider', () => {
  const loaded = loadConfig({
    NODE_ENV: 'test', APP_ORIGIN: 'http://app.test',
    AI_PROVIDER: 'mlx-local', AI_BASE_URL: 'http://127.0.0.1:8092/v1', AI_API_KEY: 'local-placeholder', AI_MODEL: 'mlx-community/Qwen3-1.7B-4bit',
    GROQ_API_KEY: 'REPLACE_WITH_ROTATED_GROQ_KEY', GROQ_AI_BASE_URL: 'https://api.groq.com/openai/v1', GROQ_AI_MODEL: 'REPLACE_WITH_SUPPORTED_GROQ_MODEL'
  });
  assert.deepEqual(loaded.ai, { provider: 'mlx-local', baseUrl: 'http://127.0.0.1:8092/v1', apiKey: 'local-placeholder', model: 'mlx-community/Qwen3-1.7B-4bit' });
});
