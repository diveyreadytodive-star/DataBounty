...
import type { AgentDescriptor, HealthResponse, StoragePublishRequest, ReviewRequest, BountyResponse, PublicBountyBoardResponse } from './api/types.js';
...
function optionalSessionAddress(request: FastifyRequest, auth: AuthService) { try { return request.cookies[COOKIE_NAME] ? auth.sessionAddress(request.cookies[COOKIE_NAME]) : null; } catch { return null; } }

export async function createApp(dependencies: AppDependencies = {}): Promise<FastifyInstance> {
  const config = dependencies.config ?? loadConfig(); const db = dependencies.db ?? new DraftProofDb(config.sqlitePath);
  const auth = new AuthService(db, config, dependencies.verifier ?? new SuiSignatureVerifier(config)); const chain = dependencies.chain ?? new LiveSuiGateway(config);
  ...
  const aiRuns = new AiRunService(db, chain, storage, seal, provider, config.packageId); const app = Fastify({ logger: { level: config.logLevel, redact: ['req.headers.authorization', 'req.headers.cookie', 'req.body.signature', 'res.headers.set-cookie'] } });
